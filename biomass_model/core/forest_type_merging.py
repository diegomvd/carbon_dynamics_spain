...
class ForestTypeMergingPipeline:
    ...
    def __init__(self, temp_dir: Optional[Path] = None):
        ...
        """
        self.logger = get_logger('biomass_estimation.forest_type_merging')
        ...
        self.biomass_patterns = ['*AGBD_MC_100m*', '*BGBD_MC_100m*', '*TBD_MC_100m*']
    ...
    def process_directory(
        ...
        Args:
            input_dir: Input directory containing forest type specific biomass rasters
            output_dir: Output directory for merged country-wide rasters
            years: Optional list of years to process
            biomass_types: Optional list of biomass types (AGBD, BGBD, TBD)
            measures: Optional list of measures (mean, uncertainty)
            resolution: Resolution string for output naming
            compression: Compression method for output GeoTIFFs
            overwrite: Whether to overwrite existing output files
            continue_on_error: Whether to continue if individual merges fail
        ...
        try:
            ...
            self.logger.info(f"Starting forest type merging...")
            ...
            biomass_dirs = self._find_biomass_directories(input_dir, biomass_types)
            ...
            for biomass_dir in biomass_dirs:
                try:
                    ...
                    years_to_process = years or self._discover_years(biomass_dir)
                    measures_to_process = measures or ['mean', 'uncertainty']
                        ...
                        for measure in measures_to_process:
                            try:
                                ...
                                success = self._merge_rasters_by_year_and_type(
                                    ...
                                    resolution, compression, overwrite
                                )
    ...
    def _discover_years(self, biomass_dir: Path) -> List[str]:
        """
        Auto-discover available years from raster files in directory.
        
        Args:
            biomass_dir: Directory containing biomass rasters
            
        Returns:
            List of discovered years as strings
        """
        years = set()
        
        # Find all raster files
        for ext in self.target_extensions:
            raster_files = list(biomass_dir.glob(f"*{ext}"))
            
            # Extract years from filenames
            for raster_file in raster_files:
                year = self._extract_year_from_filename(raster_file.name)
                if year:
                    years.add(year)
    ...
    def _merge_rasters_by_year_and_type(
        ...
        try:
            ...
            if output_file.exists() and not overwrite:
                self.logger.debug(f"Output file exists, skipping: {output_filename}")
                return True
    ...
    def validate_inputs(
        ...
        """
        Validate input directory and data.
        ...
        try:
            ...
            # Check for biomass directories
            biomass_dirs = self._find_biomass_directories(input_dir)
            if not biomass_dirs:
                self.logger.warning(f"No biomass directories found in: {input_dir}")
            else:
                self.logger.debug(f"Found {len(biomass_dirs)} biomass directories")
            
            # Check for raster files
            total_rasters = 0
            for biomass_dir in biomass_dirs:
                for ext in self.target_extensions:
                    rasters = list(biomass_dir.glob(f"*{ext}"))
                    total_rasters += len(rasters)
            
            if total_rasters == 0:
                self.logger.warning(f"No raster files found in biomass directories")
            else:
                self.logger.debug(f"Found {total_rasters} total raster files")
...
def create_forest_type_merging_pipeline(temp_dir: Optional[Path] = None) -> ForestTypeMergingPipeline:
    """
    Factory function to create a ForestTypeMergingPipeline instance.
    
    Args:
        temp_dir: Optional custom temporary directory
        
    Returns:
        Configured ForestTypeMergingPipeline instance
    """
    return ForestTypeMergingPipeline(temp_dir=temp_dir)