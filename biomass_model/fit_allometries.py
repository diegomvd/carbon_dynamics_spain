...
This script fits both height-biomass allometric relationships and BGB-AGB ratios
across hierarchical forest classifications. Creates training datasets by sampling
canopy height maps at NFI plot locations and processes all data hierarchically.
...
from fitting_utils import (
    setup_logging, load_config, validate_height_biomass_data, validate_ratio_data,
    remove_outliers, remove_ratio_outliers, process_hierarchy_levels,
    create_training_dataset, create_output_directory, prepare_ratio_data,
    AllometryResults, BGBRatioResults, TIER_NAMES
)
...
def fit_height_agb_allometry(df: pd.DataFrame, height_col: str, biomass_col: str, 
                           forest_type: str, tier: int, config: dict) -> AllometryResults:
    """
...
def calculate_bgb_ratios(df: pd.DataFrame, forest_type: str, tier: int, config: dict) -> BGBRatioResults:
    """
...
def process_hierarchical_allometries(training_df: pd.DataFrame, forest_types_df: pd.DataFrame, 
    ...
    # Define hierarchy levels to process
    hierarchy_levels = {
...
def main():
    ...
    try:
        # Load configuration
        config = load_config('config_fitting.yaml')
        
        # Load forest types hierarchy
        forest_types_df = pd.read_csv(config['data']['forest_types_path'])
        ...
        if len(allometry_df) > 0:
            allometry_path = config['output']['height_agb_path']
        ...
        if len(ratio_df) > 0:
            ratio_path = config['output']['bgb_ratios_path']
        ...
        logger.info("Allometry fitting pipeline completed")
