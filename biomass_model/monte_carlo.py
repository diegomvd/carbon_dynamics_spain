...
def monte_carlo_biomass_optimized(heights, agb_params, bgb_params, num_samples=250, seed=42):
    """
    ...
    generating mean biomass estimates and confidence intervals for AGB, BGB, and total biomass.
    ...
    """
    config = get_config()
    logger.info(f"Starting vectorized Monte Carlo simulation with {num_samples} samples")
    
    # Apply height filtering based on configuration
    logger.info("Masking heights")
    ...
    masked_heights = masked_heights.where(
        masked_heights >= config['processing']['height_threshold'], 0.0
    )
    ...
    logger.info("Calculating AGB allometry curves")
    ...
    if function_type == 'power':
        ...
        agbd_p50 = xr.where(masked_heights == 0.0, 0.0, intercept * da.exp(masked_heights * slope))
    else:
    ...
    if function_type == 'power':
        ...
        agbd_p15 = xr.where(masked_heights == 0.0, 0.0, intercept * da.exp(masked_heights * slope))
    else:
    ...
    if function_type == 'power':
        ...
        agbd_p85 = xr.where(masked_heights == 0.0, 0.0, intercept * da.exp(masked_heights * slope))
    else:
    ...
    logger.info("Generating vectorized Monte Carlo samples")
    ...
    logger.info("Generating AGB noise")
    ...
    logger.info("Generating BGB noise")
    ...
    logger.info("Broadcasting values for vectorized operations")
    ...
    logger.info("Calculating AGB samples")
    ...
    logger.info("Calculating BGB coefficients")
    ...
    logger.info("Calculating means and standard deviations")
    ...
    logger.info("Monte Carlo simulation completed - results ready for incremental computation")
    return results
